...
<div class="max-w-[1520px] 2xl:w-full py-4 mx-5 2xl:mx-auto mb-10">
    <div class="flex flex-col md:flex-row justify-center md:justify-between items-center mb-8 gap-2">
        <div class="text-center md:text-left">
            <h1 class="text-lg md:text-xl lg:text-2xl font-bold text-gray-800">Category Management</h1>
            <p class="text-xs md:text-sm text-gray-600 mt-1 lg:mt-2">Manage your product categories</p>
        </div>
        ...
        <form id="filterForm" method="GET" class="flex flex-wrap gap-4 items-center">
            ...
            <div>
                <select name="limit" id="limitFilter" class="w-16 h-12 outline-none font-semibold text-[#171717] text-sm px-3 py-2 rounded-xl border-2 border-[#f5f5f5] focus:ring-[#171717] focus-within:ring-[#171717] focus-within:border-[#171717] focus:border-[#171717] bg-[#f5f5f5]">
                    <option value="1" <?= isset($meta['limit']) && $meta['limit'] == 1 ? 'selected' : '' ?>>1</option>
        ...
        <?php else: ?>
            <div class="grid lg:grid-cols-2 gap-5">
                <?php foreach ($categories as $category): ?>
                    <div class="border border-dashed rounded-xl flex flex-col justify-center items-center md:flex-row md:justify-start gap-2 lg:gap-4 overflow-hidden p-4 relative">
                        <!-- Category Image -->
                        <div class="size-32 flex justify-center items-center bg-gray-50 rounded-lg md:shrink-0">
                            <?php if (!empty($category['image'])): ?>
                                <img src="<?= asset(htmlspecialchars($category['image'])) ?>" alt="<?= htmlspecialchars($category['name']) ?>">
                            <?php else: ?>
                                <img src="<?= asset('assets/images/empty.png') ?>" alt="">
                            <?php endif; ?>
                        ...
                        <div class="w-full md:grow">
                            <div class="pb-2 text-center md:text-left md:flex md:justify-between md:items-end gap-2">
                                <div>
                                    ...
                                    <div class="text-sm text-gray-600 mb-3 md:mb-0">
                                        <p class="truncate flex justify-center items-center md:justify-start gap-1">
                                ...
                                <div class="text-sm">
                                    <?php if (!empty($category['parent'])): ?>
                                        <p class="mt-2 md:mt-0 truncate flex justify-center items-center md:justify-start gap-1">
                                            <ion-icon name="git-branch-outline"></ion-icon>
                                            Parent: <?= htmlspecialchars($category['parent']['name']) ?>
                                        </p>
                                    <?php else: ?>
                                        <p class="mt-2 md:mt-0 text-gray-400 flex justify-center items-center md:justify-start gap-1">
                                            <ion-icon name="git-branch-outline"></ion-icon>
                                            No parent category
                                        </p>
                                    <?php endif; ?>
                            ...
                            <div class="flex justify-between items-center pt-4 border-t border-gray-100">
                                <span class="text-xs text-gray-500 bg-gray-50 px-1.5 py-1.5 leading-[1] rounded-md">
                                    ID: <?= htmlspecialchars($category['id']) ?>
                                </span>
                                <div class="flex gap-2">
                                    <a href="<?= route('categories.edit', ['id' => $category['id']]) ?>">
                                        <button class="edit-btn size-8 flex justify-center items-center bg-blue-50 hover:bg-blue-100 text-blue-600 p-2 rounded-lg transition-colors cursor-pointer"
                                            data-id="<?= htmlspecialchars($category['id']) ?>">
                                            <ion-icon name="create-outline"></ion-icon>
                                        </button>
                                    </a>
                                    <button class="delete-btn size-8 flex justify-center items-center bg-red-50 hover:bg-red-100 text-red-600 rounded-lg transition-colors cursor-pointer"
                                        data-id="<?= htmlspecialchars($category['id']) ?>"
                ...
                <div class="flex flex-col md:flex-row justify-between items-center">
                    ...
                    <div class="flex items-center space-x-2">
                        ...
                        <a href="?<?= http_build_query(array_merge($_GET, ['page' => max(1, $meta['page'] - 1)])) ?>"
                            ...
                            <ion-icon name="chevron-back-outline" class="mr-1"></ion-icon> Previous
                        </a>
                        ...
                        <a href="?<?= http_build_query(array_merge($_GET, ['page' => min($totalPages, $currentPage + 1)])) ?>"
                            ...
                            Next <ion-icon name="chevron-forward-outline" class="ml-1"></ion-icon>
                        </a>
                    </div>
                </div>
            </div>
