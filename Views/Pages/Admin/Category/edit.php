...
<div class="max-w-[1520px] 2xl:w-full py-4 mx-5 2xl:mx-auto mb-6">
    <div class="max-w-4xl mx-auto pb-10">
        ...
        <!-- Form -->
        <form method="POST" action="<?= route('categories.store.update', ['id' => $Category->id]) ?>" class="space-y-8" enctype="multipart/form-data">
            <?= csrf_field() ?>
            <?= method_field('PATCH') ?>
                    ...
                    <select
                        ...
                        <?php if (!empty($categories)): ?>
                            <?php foreach ($categories as $category): ?>
                                <option value="<?= $category['id'] ?>" <?= old('parentID', $Category->parentID) === $category['id'] ? 'selected' : '' ?>><?= $category['name'] ?></option>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </select>
                </div>
                        ...
                        <div
                            ...
                            <div id="defaultPreview" class="flex flex-col items-center justify-center h-full p-6">
                                <div class="w-20 h-20 bg-gray-200 rounded-full flex items-center justify-center mb-4">
                                    <img src="<?= asset($Category->image ?? 'assets/images/empty.png') ?>" alt="">
                                </div>
